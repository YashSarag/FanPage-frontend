...
function Fanwall(){
    ...
    async function postData({name,message,color,city,date}){
        try{
            const response = await fetch("https://fan-page-backend.vercel.app/api/v1/createFanwallCard", { 
            method: "POST",
            headers: {
    ...
    async function fetchData(){
        setLoading(true);
        try{
            const res = await fetch("https://fan-page-backend.vercel.app/api/v1/getFanwallCards");
